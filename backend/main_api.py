...
async def create_library(library_data: LibraryCreate, database: Session = Depends(get_db)) -> Library:
    ...
    db_library = Library(
        address=library_data.address,        web_page=library_data.web_page,        telephone=library_data.telephone,        name=library_data.name        )
...
async def bulk_create_library(items: list[LibraryCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_library = Library(
                address=item_data.address,                web_page=item_data.web_page,                telephone=item_data.telephone,                name=item_data.name            )
            database.add(db_library)
            database.flush()  # Get ID without committing
...
async def execute_library_cheapest_book_by(
    ...
    try:
        ...
        async def wrapper(_library_object):
            cheapest: Book = None
            price = 1000000000.0
            book_8763446370301 = (await get_books_of_library(_library_object.id, database))['books']
            for i in range(0, len(book_8763446370301)):
                book = book_8763446370301[i]
...
async def create_book(book_data: BookCreate, database: Session = Depends(get_db)) -> Book:
    ...
    db_book = Book(
        pages=book_data.pages,        release=book_data.release,        stock=book_data.stock,        price=book_data.price,        genre=book_data.genre.value,        title=book_data.title        )
...
async def bulk_create_book(items: list[BookCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_book = Book(
                pages=item_data.pages,                release=item_data.release,                stock=item_data.stock,                price=item_data.price,                genre=item_data.genre.value,                title=item_data.title            )
            database.add(db_book)
            database.flush()  # Get ID without committing
